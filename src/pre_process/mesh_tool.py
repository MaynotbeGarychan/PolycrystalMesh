...
import numpy as np
from src.pre_process.mesh_rve import rve
from src.pre_process.mesh_node import node_set
from enum import Enum
from copy import deepcopy
from tqdm import tqdm

def drag_shell_to_solid(shell_mesh: mesh, num_layers: int, thicknesss):
    """
    ...
    for i in range(shell_mesh.node_set.num):
        ret = _drag_node(shell_mesh.node_set.x_array[i], shell_mesh.node_set.y_array[i], shell_mesh.node_set.z_array[i], thickness_list)
    ...
    for i in range(shell_mesh.elem_set.num):
        ...
        # part_list
        solid_mesh.elem_set.part_list[i * num_layers:(i + 1) * num_layers] = np.full(num_layers, shell_part_id,
                                                                                     dtype=int)
